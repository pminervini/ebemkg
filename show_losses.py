...
def main(argv):
    ...
    #step = np.linspace(1, data.shape[1] + 1, data.shape[1])
    types = pd.Series(names)

    cis = np.linspace(95, 10, 4)
    ax = sns.tsplot(data, condition=types, time=step, value='Loss', color='husl', ci=cis, err_style='ci_band', interpolate=True); # ci_bars, ci_band, boot_traces (standard error)
